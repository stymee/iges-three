    ...
    iges.parameters.forEach(v => {
        switch (v.values[0]) {
            ...
            case entityTypes.Point:
                objects.push(threePoint(v, iges));
                break;

            case entityTypes.Line:
                objects.push(threeLine(v, iges));
                break;

            case entityTypes.CircularArc:
                threeCircularArc(v, iges).forEach(s => objects.push(s));
                break;
...
// export const createGeometryGroups = (iges: IgesData) => {
